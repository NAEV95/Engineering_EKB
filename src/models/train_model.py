...
def _load_model_class(params):
    model_type = params.get('type', 'lightgbm')
    if model_type == 'lightgbm':
        try:
            import lightgbm as lgb

            return lgb.LGBMRegressor, 'lightgbm'
        except (ImportError, OSError) as exc:
            logger.warning("LightGBM is unavailable (%s). Falling back to RandomForestRegressor.", exc)

    if model_type in ('lightgbm', 'random_forest'):
        from sklearn.ensemble import RandomForestRegressor

        return RandomForestRegressor, 'random_forest'
...
def train_lightgbm_model(X_train, y_train, params):
    ...
    ModelClass, backend = _load_model_class(params)
    logger.info(f"Training {backend} model")
    ...
    if backend == 'lightgbm':
        ...
        model = ModelClass(
            n_estimators=lgb_params.get('n_estimators', 200),
            max_depth=lgb_params.get('max_depth', None),
            random_state=params.get('random_seed', 42),
            n_jobs=lgb_params.get('n_jobs', -1),
        )
...
def explain_model(model, X_train, feature_names):
    ...
    try:
        ...
        logger.warning("SHAP is unavailable (%s). Skipping SHAP explanations.", exc)
        shap_values = None
        explainer = None
    else:
