...
def _load_regressor(prefer_lightgbm=True):
    if prefer_lightgbm:
        try:
            ...
            logger.warning(
                "LightGBM is unavailable (%s). Falling back to sklearn RandomForestRegressor.",
                exc,
            )
...
def _fallback_params(params, random_seed):
    return {
...
def run_lazypredict(data, output_dir, random_seed):
    ...
    try:
        ...
        logger.warning(f"LazyPredict evaluation unavailable: {str(e)}")
...
def train_model(data, model_params, output_dir, random_seed, optuna_trials=50, skip_optuna=False):
    ...
    try:
        ...
        if skip_optuna:
            ...
            Regressor, backend = _load_regressor(prefer_lightgbm=True)
            if backend != "lightgbm":
                params = _fallback_params(params, random_seed)

            logger.info(f"Training {backend} model with parameters: {params}")

            # Initialize and train model
            model = Regressor(**params)
            # Fix: verbose parameter depends on LightGBM version
            fit_signature = inspect.signature(model.fit)
            if backend == "lightgbm" and "eval_set" in fit_signature.parameters:
                try:
            ...
            Regressor, backend = _load_regressor(prefer_lightgbm=True)
            if backend != "lightgbm":
                logger.info("Skipping Optuna LightGBM optimization because LightGBM is unavailable")
                params = _fallback_params(model_params or {}, random_seed)
                model = Regressor(**params)
                model.fit(pd.concat([X_train, X_val]), pd.concat([y_train, y_val]))
                val_preds = model.predict(X_val)
                best_score = np.sqrt(mean_squared_error(y_val, val_preds))
                best_params = params
            else:
        ...
        # Save model
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        backend_name = model.__class__.__name__.lower()
        model_file = os.path.join(output_dir, f"{backend_name}_model_{timestamp}.pkl")
...
def analyze_mutation_counts(data_dir, output_dir):
    ...
    try:
        ...
        def compare_sequences(pdb_file, wild_type_pdb_file):
            ...
            aa_map = PDB.Polypeptide.protein_letters_3to1
            ...
            for chain, wild_type_chain in zip(chains, wild_type_chains):
                for residue, wild_type_residue in zip(chain, wild_type_chain):
                    ...
                    residue_resname = residue.get_resname().upper()
                    wild_type_resname = wild_type_residue.get_resname().upper()
                    if residue_resname not in aa_map or wild_type_resname not in aa_map:
                        continue

                    # Get one-letter code
                    residue_name = aa_map[residue_resname]
                    wild_type_residue_name = aa_map[wild_type_resname]
        ...
        if df.empty:
            logger.warning("No PDB files could be analyzed for mutations.")
            return None
        ...
        mutation_stats = {
            'min': int(df['mutation_count'].min()),
            'max': int(df['mutation_count'].max()),
            'mean': float(df['mutation_count'].mean()),
            'median': float(df['mutation_count'].median()),
            'std': float(df['mutation_count'].std()) if len(df) > 1 else 0.0,
            'count': int(len(df)),
            'percent_above_5': float((df['mutation_count'] >= 5).mean() * 100)
        }
